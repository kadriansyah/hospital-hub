...
class HospitalForm extends PolymerElement {
    static get template() {
        return html`
            ...
            <div id="formContainer">
                ...
                <iron-input id="id_wrapper" slot="input" bind-value="{{hospital.id}}">
                    <input id="id" type="hidden" value="{{hospital.id}}">
                </iron-input>
                
                <paper-input-container>
                    <label slot="label">Name</label>
                    <iron-input slot="input" bind-value="{{hospital.name}}">
                        <input id="name" type="text" value="{{hospital.name}}">
                    </iron-input>
                </paper-input-container>
                
                <paper-input-container>
                    <label slot="label">Description</label>
                    <iron-input slot="input" bind-value="{{hospital.description}}">
                        <input id="description" type="text" value="{{hospital.description}}">
                    </iron-input>
                </paper-input-container>
    ...
    _onEditResponse(data) {
        ...
        this.hospital = response.payload;
        if (this._mode === 'copy') {
            this.hospital.id = ''; // nullify id, we will save it as new document
        }
        this.dispatchEvent(new CustomEvent('editSuccess', {bubbles: true, composed: true}));
    }

    _onEditError() {
        this._error = 'Edit Hospital Error';
    }
    ...
    _onSaveResponse(e) {
        ...
        if (response.status == '200') {
            ...
            this._error = 'Creating Hospital Error';
        }
    ...
    _onUpdateResponse(data) {
        ...
        if (response.status == '200') {
            ...
            this._error = 'Updating Hospital Error';
        }
        this.$.progress.hidden = true;
    }
    ...
    _save() {
        if (this._mode === 'new' || this._mode === 'copy') {
            ...
            this.$.updateAjax.body = this.hospital;
            this.$.updateAjax.url = this.actionUrl +'/'+ this.hospital.id;
...
customElements.define('hospital-form', HospitalForm);
